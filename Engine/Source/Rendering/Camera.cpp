...
void Camera::PanForward(float delta)
{
    ...
    float moveScale = m_distance * 0.001f;
    glm::vec3 offset = forward * delta * moveScale;
...
{
    // Zoom by adjusting distance (dolly toward/away from target)
...
void Camera::FrameBounds(const glm::vec3& center, float radius)
{
    ...
    m_distance = (radius * 2.0f) / halfFovTan;
    ...
    m_nearPlane = std::max(0.001f, radius * 0.001f);
    m_farPlane = std::max(1000.0f, m_distance * 20.0f);
